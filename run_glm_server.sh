#!/bin/bash
# DEFAULT GLM 5.2 API server: pipeline + SSD streaming + fp8 KV.
# No tensor parallelism. TP lives on the tp-cuda-server branch: measured slower
# (52.5 vs 77.2 t/s prefill at a matched 500k allocation and identical 98k prompt)
# and capped at half the context, so it is not the default here.
#
...
#
# WHY PIPELINE RATHER THAN TENSOR PARALLEL
# -------------------------------------
